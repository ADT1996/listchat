  ...
  Future<dynamic> post(String uri,
    ...
    if (headers != null) {
      if (!headers.containsKey(Header.CONTENT_TYPE)) {
        headers.addAll({Header.CONTENT_TYPE: Header.APP_JSON});
      }
    } else {
      headers = {Header.CONTENT_TYPE: Header.APP_JSON};
    }

    print('API to -> ${_URI + uri}');

    final response =
        await http.post(_URI + uri, body: json.encode(body), headers: headers);
    switch (response.statusCode) {
      case 200:
        return json.decode(response.body);
      default:
